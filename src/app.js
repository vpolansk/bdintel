...
function localTipoLabel(tipo) {
  return {
    poi: 'PONTO DE INTERESSE',
    recorrente: 'LOCAL RECORRENTE',
    trafico: 'TRAFICO',
    fuga: 'ROTA DE FUGA',
    esconderijo: 'ESCONDERIJO / APOIO',
    reuniao: 'PONTO DE REUNIAO',
  }[tipo] || String(tipo || 'LOCAL').toUpperCase();
}

function localRisco(l) {
  if (['trafico', 'esconderijo', 'fuga'].includes(l.tipo)) return { label: 'RISCO ALTO', cls: 'risk-high' };
  if (l.tipo === 'recorrente' || l.tipo === 'reuniao') return { label: 'RISCO MEDIO', cls: 'risk-med' };
  return { label: 'MONITORAR', cls: 'risk-low' };
}
...
function distMeters(aLat, aLng, bLat, bLng) {
  if (![aLat, aLng, bLat, bLng].every(n => typeof n === 'number' && !isNaN(n))) return Infinity;
  const R = 6371000;
...
function isRelacionadoAoLocal(l, item, campos = []) {
  const nome = txtNorm(l.nome);
  const endereco = txtNorm(l.endereco);
  const texto = txtNorm(campos.map(c => item[c]).filter(Boolean).join(' '));
  const porTexto = texto && ((nome && texto.includes(nome)) || (endereco && texto.includes(endereco.slice(0, 20))));
  const porCoord = l.lat && l.lng && item.lat && item.lng && distMeters(Number(l.lat), Number(l.lng), Number(item.lat), Number(item.lng)) <= 350;
  return porTexto || porCoord;
}

function getLocalDossie(l) {
  const pessoas = DB.pessoas.filter(p =>
    isRelacionadoAoLocal(l, p, ['endereco', 'bairro', 'cidade', 'obs', 'vinculosInfo']) ||
    (p.eventos || []).some(ev => isRelacionadoAoLocal(l, ev, ['local', 'historico']))
  );
  const veiculos = DB.veiculos.filter(v =>
    isRelacionadoAoLocal(l, v, ['obs', 'infoCompartilhamento', 'proprietario', 'condutor']) ||
    (v.eventos || []).some(ev => isRelacionadoAoLocal(l, ev, ['local', 'historico']))
  );
  const ocorrencias = (DB.ocorrencias || []).filter(oc =>
    isRelacionadoAoLocal(l, oc, ['local', 'historico', 'ba', 'veiculos']) ||
    (oc.andamentos || []).some(a => isRelacionadoAoLocal(l, a, ['local', 'texto', 'notas', 'veiculos']))
  );
    ...
    <div class="local-dossier">
      ...
      <section class="local-card-panel">
        <details class="local-fold" open>
          <summary>Pessoas vinculadas (${dossie.pessoas.length})</summary>
      ...
      <section class="local-card-panel">
        <details class="local-fold" open>
          <summary>Veiculos / apoio (${dossie.veiculos.length})</summary>
          <div class="local-mini-list">
            ${dossie.veiculos.length ? dossie.veiculos.slice(0, 6).map(v => `
              <div class="local-mini-item" onclick="goPage('veiculos');selectVeiculo('${v.id}')">
                <div class="mini-plate">${v.placa || 'PLACA'}</div>
                <div><b>${[v.marca, v.modelo].filter(Boolean).join(' ') || 'Veiculo'}</b><span>${v.cor || ''} ${statusBadge(v.status || 'cadastrado')}</span></div>
              </div>`).join('') : '<div class="empty-state">Nenhum veiculo relacionado ainda</div>'}
          </div>
        </details>
      </section>
      ...
      <section class="local-card-panel">
        <details class="local-fold" open>
          <summary>Ocorrencias e historico (${dossie.ocorrencias.length})</summary>
          <div class="local-timeline">
            ${dossie.ocorrencias.length ? dossie.ocorrencias.slice(0, 6).map(oc => `
              <div class="tl-card" onclick="goPage('ocorrencias');selectOcorrencia('${oc.id}')">
                <div class="tl-local">${tipoLabel(oc.tipo || 'ocorrencia')} - ${fmtDate(oc.data)}</div>
                <div class="tl-desc">${oc.historico || oc.local || '-'}</div>
              </div>`).join('') : '<div class="empty-state">Nenhuma ocorrencia vinculada ainda</div>'}
          </div>
        </details>
      </section>
